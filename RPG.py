...
class joc:
    ...
    def crear_personatge(self):
        ...
        if final == "reset":
            ...
            self.personatges.append(personatge(nom,raça,edat,força,destresa,constitucio,inteligencia,saviesa,carisma))
            ...
            match opcio:
                case 1:
                    ...
                    for p in self.personatges:
                        if i + 1 != numero_jugador1:
                            ...
                            if comptador == numero_jugador2:
                                ...
                                break
                        i += 1
                    print(f"\nPvP: {jugador1.nom} VS {jugador2.nom}")
                    print(f"{jugador1.nom} - Vida: {jugador1.vida}/{jugador1.vida_maxima} | Maná: {jugador1.mana}/{jugador1.mana_maxim}")
                    print(f"{jugador2.nom} - Vida: {jugador2.vida}/{jugador2.vida_maxima} | Maná: {jugador2.mana}/{jugador2.mana_maxim}")
                    ...
                    while jugador1.vida > 0 and jugador2.vida > 0:
                        ...
                        print(f"\n--- TORN DE {actual.nom} (Vida: {actual.vida}/{actual.vida_maxima} | Maná: {actual.mana}/{actual.mana_maxim}) ---")
                        print("1. Atacar")
                        ...
                        accio = self.llegir_enter("Tria acció: ")
                        
                        match accio:
                            case 1:
                                if actual.arma_equipada:
                                    if actual.arma_equipada.tipus == "màgia":
                                        dany = (actual.arma_equipada.dany * actual.inteligencia) / 100
                                    else:
                                        bonus_arma = actual.arma_equipada.dany / 100 if actual.arma_equipada else 0
                                        dany = actual.força * (1 + bonus_arma)
                                else:
                                    dany = actual.força
                                
                                dany = int(dany)
                                
                                if defensa_activa[1-idx_defensa]:
                                    prob_esquiva = max(0, (enemic.destresa - 5) * 3.33)
                                    if random.randint(1, 100) <= prob_esquiva:
                                        print(f"{enemic.nom} ESQUIVA l'atac!")
                                        defensa_activa[1-idx_defensa] = False
                                        torn = 3 - torn
                                        continue
                                
                                if defensa_activa[1-idx_defensa]:
                                    dany //= 2
                                    print(f"{enemic.nom} està defensant-se!")
                                    defensa_activa[1-idx_defensa] = False
                            ...
                            case 2:
                                regeneracio = actual.constitucio * 3
                                actual.vida = min(actual.vida_maxima, actual.vida + regeneracio)
                                print(f"{actual.nom} regenera {regeneracio} de vida (ara: {actual.vida}/{actual.vida_maxima})")
                            
                            case 3:
                                regeneracio_mana = actual.inteligencia * 2
                                actual.mana = min(actual.mana_maxim, actual.mana + regeneracio_mana)
